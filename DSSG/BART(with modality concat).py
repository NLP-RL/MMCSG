...
class MAF_KG(nn.Module):
    ...
    def forward(self,
                text_input: torch.Tensor,
                visual_context: Optional[torch.Tensor]=None,
                audio_context: Optional[torch.Tensor]=None, ppc_context: Optional[torch.Tensor]=None):
        ...
        # Global Information Fusion Mechanism
        weight_audio = F.sigmoid(self.kg_gate(torch.cat((audio_out, text_input), dim=-1)))
        weight_v = F.sigmoid(self.visual_gate(torch.cat((video_out, text_input), dim=-1)))
        weight_ppc = F.sigmoid(self.ppc_gate(torch.cat((ppc_out, text_input), dim=-1)))

        output = self.final_layer_norm(text_input + weight_audio * audio_out + weight_ppc * ppc_out +
                                       weight_v * video_out)
...
class MultimodalBartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.visual_input_length_transform = nn.Linear(1, SOURCE_MAX_LEN)
        self.audio_input_length_transform = nn.Linear(1, SOURCE_MAX_LEN)
        self.ppc_input_length_transform = nn.Linear(14, SOURCE_MAX_LEN)

        self.concat_linear = torch.nn.Linear(2*embed_dim + ACOUSTIC_DIM + VISUAL_DIM, embed_dim)
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusionVis_at_layer:
                ...
                print("ppc shape up: ", ppc_input.shape)
                print("hidden_states shape up: ", hidden_states.shape)
                print("audio_input shape up: ", audio_input.shape)
                print("visual_input shape up: ", visual_input.shape)
                ...
                visual_input = self.visual_input_length_transform(visual_input)
                visual_input = visual_input.permute(0,2,1)

                audio_input = audio_input.permute(0,2,1)
                audio_input = self.audio_input_length_transform(audio_input)
                audio_input = audio_input.permute(0,2,1)

                ppc_input = ppc_input.permute(0,2,1)
                print("ppc shape : ", ppc_input.shape)
                ppc_input = self.ppc_input_length_transform(ppc_input)
                print("ppc shape up: ", ppc_input.shape)
                ppc_input = ppc_input.permute(0,2,1)
                ...
                # print("Text shape : ", hidden_states.shape)
                # print(1/0)
                ...
                # a1= torch.cat((hidden_states, audio_input, visual_input, ppc_input), dim = -1)
                a1= torch.cat((hidden_states, audio_input, visual_input, ppc_input), dim = -1)
                print("a1 shape : ", a1.shape)
                hidden_states = self.concat_linear(a1)
...
def prepare_for_training(model,
    ...
    for name, param in model.named_parameters():
        if "audio_transformer" in name or "visual_transformer" in name or "ppc_transformer" in name or "MAF_layer" in name:
