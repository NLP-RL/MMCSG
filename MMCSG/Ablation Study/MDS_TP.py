...
class MAF(nn.Module):
    ...
    def __init__(self,
        ...
        self.PPCC_transform = nn.Linear(PPCC_MAX_LEN, SOURCE_MAX_LEN, bias=False)
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if idx in self.fusion_at_layer:
                print("ppcc input shape here: ", ppcc_input.shape)

                pppc_input = self.ppcc_transformer(ppcc_input)[-1]
...
def val_epoch(model,
    ...
    with torch.no_grad():
        for step, batch in enumerate(tqdm(data_loader, desc="Validation Loss Iteration")):
            ...
            input_ids, attention_mask, personality_input, labels = batch
            print("personality input shape validation : ", personality_input.shape)
...
def get_val_scores(model,
    ...
    if "Test" in desc:
        ...
        file_name = "./Gen/" + str(epoch+1) + "_test_MDS_TP_results.csv"
        test_df.to_csv(file_name, index=False)    
    print("Test File saved")
...
def prepare_for_training(model,
    ...
    for name, param in model.named_parameters():
        if "acoustic_transformer" or "ppcc_transformer" or "MAF_layer" in name:
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        print("personality input shape train : ", personality_input.shape)
        personality_input = personality_input.squeeze(dim = 1)
        print("personality input shape train : ", personality_input.shape)
